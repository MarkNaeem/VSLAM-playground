...
MAX_IMG_HEIGHT = 256
MAX_DEPTH = 50
TRAJECTORY_PATH = 'trajectories/feature_lidar_icp_trajectory.npy'
...
lidar_path = f'{DATASET_PATH}/sequences/{DATA_TRACK}/velodyne/{len(trajectory)-1:06d}.bin'
lidar_points = read_velodyne_bin(lidar_path)
prev_image_points, prev_depths = project_lidar_to_camera(lidar_points, extrinsic_matrix, intrinsic_matrix, image_width, image_height)
prev_lidar_depth_image, _ = create_depth_image(prev_image_points, prev_depths, image_width, image_height)
prev_depth = densify_depth_image_fast(prev_lidar_depth_image)
...
for i in tqdm(range(len(trajectory),num_images - 1)):
  try:
    ...
    valid = matches > -1
    mkpts0 = kpts0[valid].astype(int)
    mkpts1 = kpts1[matches[valid]].astype(int)

    pts13d, deleted = keypoints_to_3d(mkpts1,depth,intrinsic_matrix,MAX_DEPTH)
    mkpts0 = np.delete(mkpts0,deleted,0)
    pts03d, deleted = keypoints_to_3d(mkpts0,prev_depth,intrinsic_matrix,MAX_DEPTH)
    pts13d = np.delete(pts13d,deleted,0)
    
    transformation_matrix = run_icp(pts03d,pts13d)
